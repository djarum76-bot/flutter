...
class BCA{
  ...
  Future<int> isoIsMyCard(Uint8List cardNo, Uint8List cardInfo) async {
    ...
    try {
      var respData = await FlutterNfcKit.transceive(apduClSelectApp);
      if (_resultCode(respData, respData.length) != _resultOK) {
        return -1;
      }

      respData = await FlutterNfcKit.transceive(apduClFastSelectFile);
      if (_resultCode(respData, respData.length) != _resultOK) {
        return -1;
      }

      respData = await FlutterNfcKit.transceive(apduClFastGetBalance);
      if (_resultCode(respData, respData.length) != _resultOK) {
        return -1;
      }
      ...
      respData = await FlutterNfcKit.transceive(apduClFastGetInfo);
      if (_resultCode(respData, respData.length) != _resultOK) {
        return -1;
      }
